...
-- [回答]select store_name, goods_name,quantity from stock_table join store_table on stock_table.store_code = store_table.store_code 
join goods_table on stock_table.goods_code = goods_table.goods_code;
 
 
-- 4. 商品テーブルから全商品の単価の平均値を抽出しなさい。
-- [回答]select ceil(avg(price)) from goods_table
 
 
-- 5. 店舗コード='EA01'の在庫数の平均値より大きい在庫数を持つ店舗コードを抽出しなさい。
-- [回答]select store_code from stock_table where quantity > (select avg(quantity) from stock_table where
										   store_code = 'EA01')
...
values('M001', 'マフラー', 4500, '2021-03-03');
...
-- [回答]
	...
	set 
	quantity = 10,
	update_day = '2021-03-03'
	where goods_code = 'S987' and store_code ='EA01'
 
 
-- 8. 7で更新した商品を削除しなさい。※実行後のSELECT結果も貼付すること。(stock_tableからすべてのS987を削除しました)
-- [回答] delete from stock_table where goods_code = 'S987'